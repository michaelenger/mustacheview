...
class Mustache {
	...
	protected $_throwsExceptions = array(
		Exception::UNKNOWN_VARIABLE         => false,
		Exception::UNCLOSED_SECTION         => true,
		Exception::UNEXPECTED_CLOSE_SECTION => true,
		Exception::UNKNOWN_PARTIAL          => false,
		Exception::UNKNOWN_PRAGMA           => true,
	);
		...
		if (isset($options['pragmas'])) {
			foreach ($options['pragmas'] as $pragma_name => $pragma_value) {
				if (!in_array($pragma_name, $this->_pragmasImplemented, true)) {
					throw new Exception('Unknown pragma: ' . $pragma_name, Exception::UNKNOWN_PRAGMA);
				}
			}
	...
	protected function _findSection($template) {
			...
			switch ($type) {
				...
				case '/':
					if (empty($section_stack) || ($tag_name !== array_pop($section_stack))) {
						if ($this->_throwsException(Exception::UNEXPECTED_CLOSE_SECTION)) {
							throw new Exception('Unexpected close section: ' . $tag_name, Exception::UNEXPECTED_CLOSE_SECTION);
						}
					}
		...
		if (!empty($section_stack)) {
			if ($this->_throwsException(Exception::UNCLOSED_SECTION)) {
				throw new Exception('Unclosed section: ' . $section_stack[0], Exception::UNCLOSED_SECTION);
			}
		}
	}
	 ...
	 * @param mixed $matches
	 * @return void
	 * @throws Exception unknown pragma
	 */
	 ...
	 *
	 * Expects exceptions to be Exception error codes (i.e. class constants).
	 *
	 * @access protected
	 ...
	 * @throws Exception Unmatched section tag encountered.
	 * @return string
	 */
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws Exception Unknown variable name.
	 * @return string
	 */
	...
	protected function _findVariableInContext($tag_name, $context) {
		...
		if ($this->_throwsException(Exception::UNKNOWN_VARIABLE)) {
			throw new Exception("Unknown variable: " . $tag_name, Exception::UNKNOWN_VARIABLE);
		} else {
			return '';
		}
	}
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws Exception Unknown partial name.
	 * @return string
	 */
	protected function _getPartial($tag_name) {
		...
		if ($this->_throwsException(Exception::UNKNOWN_PARTIAL)) {
			throw new Exception('Unknown partial: ' . $tag_name, Exception::UNKNOWN_PARTIAL);
		} else {
			return '';
		}
	}
